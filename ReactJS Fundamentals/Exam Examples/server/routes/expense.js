...
module.exports = {
    postExpense: (req, res) => {
        ...
        if (!expense.year) {
            expense.year = Number(req.params.year);
        }
        if (!expense.month) {
            expense.month = Number(req.params.month);
        }
...
function validateExpense(expense) {
    ...
    if (expense.paymentDate !== parseInt(expense.paymentDate)) {
        isFormValid = false;
        errors.paymentDate = 'Date must be a number.';
    }
    ...
    if (expense.paymentDate < 1 || expense.paymentDate > 31) {
        isFormValid = false;
        errors.paymentDate = 'Date must be an integer between 1 and 31.';
    }
