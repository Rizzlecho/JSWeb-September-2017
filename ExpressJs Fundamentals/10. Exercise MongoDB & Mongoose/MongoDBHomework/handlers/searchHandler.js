...
let fixDate = (images) => {
    for (let image of images) {
        image.paymentDate = image.paymentDate.setHours(3);
        image.paymentDate = image.paymentDate.setMinutes(0);
        image.paymentDate = image.paymentDate.setSeconds(0);
        image.paymentDate = image.paymentDate.setMilliseconds(0);
    }
}
        ...
        if (tagInput !== "Write tags separted by ," && beforeDate && afterDate) {
            Tag.findOne({ tagName: tagInput }).then((tag) => {
                ...
                Image
                    ...
                    .then((images) => {
                        fixDate(images);
                        images = images.filter(d => d.paymentDate > maxDate && d.paymentDate < minDate);
            ...
            Tag.findOne({ tagName: tagInput }).then((tag) => {
                ...
                Image
                    ...
                    .then((images) => {
                        fixDate(images);
                        images = images.filter(d => d.paymentDate < minDate);
                        displayHtml(res, images);
                    });
            }).catch((err) => {
                console.log(err);
            ...
            Tag.findOne({ tagName: tagInput }).then((tag) => {
                ...
                Image
                    ...
                    .then((images) => {
                        fixDate(images);
                        images = images.filter(d => d.paymentDate > maxDate);
                        displayHtml(res, images);
                    })
            }).catch((err) => {
                console.log(err);
